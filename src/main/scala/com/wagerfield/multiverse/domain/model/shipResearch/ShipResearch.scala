 ...
 */
case class ShipResearch private(uncommittedEvents: List[ShipResearchEvent], speciesId:SpeciesId)
  extends AggregateRoot[ShipResearch, ShipResearchEvent]
{
   ...
   */
  def markCommitted: ShipResearch = copy(uncommittedEvents = Nil)
   ...
   */
  def applyEvent(event: ShipResearchEvent): ShipResearch = unhandled(event)
}
